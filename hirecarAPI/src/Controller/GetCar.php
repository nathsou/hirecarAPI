...
class GetCar extends RequetageBDD
{
    ...
    public function get_parking_lots(Request $request){
        ...
        if ((isset($center_lat) && isset($center_lng) && isset($radius)) xor isset($airport)){
            if(isset($center_lat) && isset($center_lng) && isset($radius) && is_numeric($center_lat)&& is_numeric($center_lng) && is_numeric($radius)){
                $this->query .="(SQRT(POW((`lat`-:lat),2)+POW((`lng`-:long),2))) < :radius ";
                $this->queryParameter["lat"]=$center_lat;
                $this->queryParameter["long"]=$center_lng;
                ...
                {return $this->json(["error"=>"data parameter are unvalible"]);}
            $this->selectByPrice([
            ...
            $this->selectBynbplaces($request->query->get("nbPlaces"));
            $this->selectBynbporte($request->query->get("nbPorte"));
            ...
            $prep->execute();
            return $this->json(["car"=>$prep->fetchAll(\PDO::FETCH_ASSOC),"query"=>$this->query,"query parameter"=>$this->queryParameter]);
    ...
    private function selectBynbplaces($nbPlaces){
        if (isset($nbPlaces)&& is_numeric($nbPlaces)){
            $this->query.= "AND nb_places >= :nbPlaces ";
            $this->queryParameter["nbPlaces"]=$nbPlaces;
        }
    }
    private function selectBynbporte($nbPorte){
        if (isset($nbPorte)&& is_numeric($nbPorte)){
            $this->query.= "AND nb_places >= :nbPorte ";
            $this->queryParameter["nbPorte"]=$nbPorte;
        }
    }
